definition(
    name: "RPi Monitor",
    namespace: "asishvarghese/rpi_monitor",
    author: "Asish Varghese",
    description: "RPi Security Monitor",
...
def selectPiPage() {
    ...
    sendHubCommand(new physicalgraph.device.HubAction("lan discovery urn:schemas-upnp-org:device:RPi_Monitor:", physicalgraph.device.Protocol.LAN))
...
def locationHandler(evt) {
    ...
    log.debug('RPi Monitor App Received Response: ' + evt.description)
    ...
    if (parsedEvent?.ssdpTerm?.contains("schemas-upnp-org:device:RPi_Monitor:")) {
        def devices = getDevices()
...
def initialize() {
    ...
    selectedRPi.each { ssdpUSN ->
        ...
        if (!d) {
            ...
            if (ssdpUSN.endsWith(":100")) // Hack to detect if the device is a motion or contact type
            {
                d = addChildDevice("asishvarghese/rpi_motion", "RPi Motion Sensor", dni, devices[ssdpUSN].hub, [
            ...
            {
                d = addChildDevice("asishvarghese/rpi_monitor", "RPi Home Monitor", dni, devices[ssdpUSN].hub, [
                    "label": convertHexToIP(ip) + ':' + convertHexToInt(port),
                    "data": [
                        "ip": ip,
                        "port": port,
    ...
    unschedule()
    schedule("0 30 * * * ?", subscribeToDevices)
    subscribeToDevices()
}
